...
# Variable global para guardar el último estado enviado
last_state = None
...
def detect_objects(interpreter, image, threshold_min=0.62, threshold_max=0.90):
    ...
    for i in range(count):
        if threshold_min <= scores[i] <= threshold_max:
            result = {
...
def run_odt_and_draw_results(image_path, interpreter, threshold_min=0.5, threshold_max=0.65):
    ...
    if results:
        for obj in results:
            ...
            # Determinar el estado basado en la posición del objeto
            ...
            # Solo necesitamos un estado por cuadro
            break
    else:
        current_state = b'0\n'  # Apagado si no hay detecciones

    # Enviar el estado solo si cambia
...
while True:
    ...
    detection_result_image = run_odt_and_draw_results(
        temp_image_path, interpreter, threshold_min=0.6, threshold_max=0.75
    )
