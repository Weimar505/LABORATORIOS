...
#include "utils/uartstdio.h"     // Para usar uart printf
...
static void configureUART4(void);
static void config_gpio(void);

// Función principal
int main(void) {
    configureSysClock();  // Configurar el sistema a 120 MHz
    configureUART4();      // Configuración del UART4
    config_gpio();        // Configuración de los GPIOs

    // Bucle principal
    while (1) {
        UARTprintf("hola\n");  // Enviar mensaje a través del UART4
        SysCtlDelay(ui32SysClock * 0.2);  // Retraso
    }
}

// Manejador de la interrupción de UART4 (cuando se recibe un dato)
void UART4IntHandler(void) {
    ...
    while (UARTCharsAvail(UART4_BASE)) {
        ...
        receivedChar = UARTCharGetNonBlocking(UART4_BASE);
        ...
        if (receivedChar == '\r' || receivedChar == '\n') {
            ...
            UARTprintf("Datos recibidos UART4: ");
...
static void configureUART4(void) {
    ...
    SysCtlPeripheralEnable(SYSCTL_PERIPH_UART4);  // Habilitar el reloj para UART4
    
    // Configurar los pines PA0 (RX) y PA1 (TX) para UART4
    GPIOPinConfigure(GPIO_PA2_U4RX);  // Configura PA0 como RX de UART4
    GPIOPinConfigure(GPIO_PA3_U4TX);  // Configura PA1 como TX de UART4
    GPIOPinTypeUART(GPIO_PORTA_BASE, GPIO_PIN_2 | GPIO_PIN_3);  // Configura los pines como UART

    // Configurar UART4: 9600 baudios, 8 bits de datos, sin paridad, 1 bit de parada
    UARTConfigSetExpClk(UART4_BASE, ui32SysClock, BAUDIOS,
                        (UART_CONFIG_WLEN_8 |
                         UART_CONFIG_STOP_ONE |
                         UART_CONFIG_PAR_NONE));

    // Habilitar interrupciones de recepción en UART4
    IntEnable(INT_UART4);  // Habilitar la interrupción en el procesador
    UARTIntEnable(UART4_BASE, UART_INT_RX | UART_INT_RT);  // Habilitar interrupciones RX y de timeout
    IntMasterEnable();     // Habilitar interrupciones globales

    // Inicializar la UART estándar para printf
    UARTStdioConfig(4, BAUDIOS, ui32SysClock);  // Configuración de UART4 para printf
}
